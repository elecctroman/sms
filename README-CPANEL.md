...
4. Sistem gereksinimleri kontrolünü geçtikten sonra veritabanı bilgilerinizi girin ve kurulumu tamamlayın. Kurulum sonunda `install` klasörü otomatik olarak yeniden adlandırılır.
...
- 2FA adımı e-posta OTP ile sağlanır (mail() fonksiyonu kullanılır).
...
- **E-posta OTP gönderilmiyor:** Paylaşımlı hosting sağlayıcınız `mail()` fonksiyonunu devre dışı bırakmışsa uygulama hata vermeden sizi bilgilendirir. Bu durumda `config/config.php` dosyasındaki `mail.smtp.*` alanlarını doldurarak SMTP ile gönderimi etkinleştirebilirsiniz.
