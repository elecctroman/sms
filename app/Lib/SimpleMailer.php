...
final class SimpleMailer
{
    ...
    private function sendViaSmtp(string $to, string $message, string $headers, string $from): bool
    {
        ...
        if (str_contains($host, 'example.com')) {
            $this->lastError = 'SMTP sunucusu placeholder değerinde. Lütfen config/config.php dosyasında gerçek SMTP host bilgilerini girin.';
            $this->log($this->lastError);
    ...
    private function hasConfiguredSmtpHost(): bool
    {
        $host = (string) Config::get('mail.smtp.host', '');
